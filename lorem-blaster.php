...
class Lorem_Blaster {
	...
	public function __construct() {
		...
		add_action( 'admin_post_lorem_blaster_generate', array( $this, 'handle_generate' ) );
	...
	public function render_admin_page() {
		...
		<div class="lb-wrap">
			...
			<div class="lb-title-wrap">
				...
				<span class="lb-author-tag"><?php esc_html_e( 'By Naveen Kharwar', 'lorem-blaster' ); ?></span>
			</div>
			...
			<div class="lb-notice lb-notice--spaced">
				<strong><?php esc_html_e( 'Important notes before generating content', 'lorem-blaster' ); ?></strong>
				<ul>
					<li><?php esc_html_e( 'Some content sources rely on external APIs and may fall back to classic Lorem Ipsum if unavailable.', 'lorem-blaster' ); ?></li>
					<li><?php esc_html_e( 'Images are fetched from public placeholder services and stored locally in your Media Library.', 'lorem-blaster' ); ?></li>
					<li><?php esc_html_e( 'Character limits are approximate to preserve paragraph and block structure.', 'lorem-blaster' ); ?></li>
				</ul>
			</div>
			...
			if ( $last_run && is_array( $last_run ) ) :
				...
				?>
				<div class="lb-notice">
					<strong><?php esc_html_e( 'Content generated successfully', 'lorem-blaster' ); ?></strong>
					<ul>
			...
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="lb-card">
				...
				<div class="lb-grid">
					...
					<div>
						...
						<select name="source">
							<option value="lorem"><?php esc_html_e( 'Lorem Ipsum (offline safe)', 'lorem-blaster' ); ?></option>
							<option value="bacon"><?php esc_html_e( 'Bacon Ipsum (API)', 'lorem-blaster' ); ?></option>
							<option value="hipster"><?php esc_html_e( 'Hipster Ipsum (API)', 'lorem-blaster' ); ?></option>
							<option value="dummyjson"><?php esc_html_e( 'DummyJSON (API)', 'lorem-blaster' ); ?></option>
						</select>
						<p class="description">
							<?php esc_html_e(
				...
				<div class="lb-checkbox">
					...
					<label>
						...
						<?php esc_html_e( 'Automatically assign taxonomies and tags', 'lorem-blaster' ); ?>
					</label>
	...
	public function handle_generate() {
		...
		if ( '' === $raw_nonce || ! wp_verify_nonce( $raw_nonce, 'lorem_blaster_nonce' ) ) {
		...
		for ( $i = 0; $i < $count; $i++ ) {
			...
			$text               = mb_substr( trim( $this->get_content( $source, $char_limit, $used_text_fallback ) ), 0, $char_limit );
			...
			if ( $content_image || $featured_image ) {
				...
				$image_failed        = false;
				$image_id            = $this->download_image( $image_src, $used_image_fallback, $image_failed );
			...
			$blocks = $this->build_block_content( $text, $content_image, $image_id );
		...
		set_transient( 'lorem_blaster_last_run', $created_posts, MINUTE_IN_SECONDS );
		set_transient( 'lorem_blaster_last_run_notices', $run_notices, MINUTE_IN_SECONDS );
	...
	private function get_content( $source, $char_limit, &$used_fallback = false ) {
		...
		$fallback_seed = __( 'Lorem ipsum dolor sit amet, consectetur adipiscing elit.', 'lorem-blaster' );
		$fallback      = trim(
			str_repeat(
				$fallback_seed . ' ',
				max( 1, (int) ceil( $char_limit / max( 1, mb_strlen( $fallback_seed ) ) ) )
			)
		);
		...
		$urls = array(
			'bacon'     => 'https://baconipsum.com/api/?paras=' . $desired_paras . '&type=meat-and-filler',
			'hipster'   => 'https://hipsum.co/api/?paras=' . $desired_paras . '&type=hipster-centric',
		...
		for ( $attempt = 0; $attempt < $max_attempts; $attempt++ ) {
			...
			if ( '' !== $chunk ) {
				$combined = '' === $combined ? $chunk : $combined . "\n\n" . $chunk;
			}
		...
		if ( is_array( $data ) ) {
			if ( isset( $data['posts'] ) && is_array( $data['posts'] ) ) {
				...
				foreach ( $data['posts'] as $post ) {
					if ( is_array( $post ) && isset( $post['body'] ) && is_string( $post['body'] ) ) {
	...
	private function build_block_content( $text, $include_image, $image_id ) {
		...
		foreach ( preg_split( "/\n\s*\n/", $text ) as $para ) {
			$output .= "<!-- wp:paragraph -->\n<p>" . esc_html( trim( $para ) ) . "</p>\n<!-- /wp:paragraph -->\n";
		}

		if ( $include_image && $image_id ) {
			$url = wp_get_attachment_url( $image_id );
			$output .= "<!-- wp:image -->\n<figure class=\"wp-block-image\"><img src=\"" . esc_url( $url ) . "\" alt=\"\" /></figure>\n<!-- /wp:image -->\n";
		}
	...
	private function assign_auto_terms( $post_id, $post_type ) {
		...
		foreach ( get_object_taxonomies( $post_type ) as $taxonomy ) {
			wp_set_object_terms(
				$post_id,
				array( 'Sample', 'Demo', ucfirst( $post_type ) ),
				$taxonomy,
				false
			);
		}
